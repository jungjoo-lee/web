...
-- booklist outprice 필드의 이름을 rentprice로 수정하세요
...
-- memberlist 의 gender 필드에 'M' 또는 'F' 줄둥에 하나만 입력되게 제약조건을 설정한다면
...
-- gender in ('M', 'F') : gender 필드값이 in 함수 안에 있는 값들일때만 true를 결과로
...
alter table ORDERS1 add constraint check_Promotion_ID check (Promotion_ID >= 10000 AND Promotion_ID <= 99999);
alter table ORDERS1 add constraint check_Promotion_ID check (Promotion_ID between 10000 AND 99999);
