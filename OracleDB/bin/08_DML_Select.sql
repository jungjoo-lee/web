...
-- select의 사용법
-- select : select와 from 사이에 조회하고자 하는 필드명을 ,로 구분하여 나열합니다.
...
-- 출판년도가 2020년 이후인 도서의 제목과 출파년ㄴ도를 조회
...
select * from memberlist where birth > '1983/12/31';
...
-- ANY() : 괄호안에 나열된 내용중 어느 하나라도 해당하는 것이 있다면 검색 대상으로 함
-- in과 유사
...
-- 괄호안의 모든값이 동시 만족해야하는 조건이므로 해당하는 레코드가 없을때가 대부분입니다. 사용 빈도수가 현저히 낮습니다

select * from emp where deptno <> ALL(10, 20, 40);
-- 위와 같이 구성내용과 모두 같지 않을때를 필터링할때 자주 사용합니다.
-- 위 예문을 in 으로 표현
...
-- 위의 예제로 봤을때 job으로 1차 내림차순 정렬하고, 같은 job 값들 사이에 hiredate로 내림차순 정렬합니다.